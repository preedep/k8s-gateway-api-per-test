...
MODE="${1:-}"
if [[ -z "${MODE}" ]]; then
  err "Usage: bash perf-routing/50-loadtest-fortio.sh <nginx|envoy|istio|kong> [duration] [concurrency] [qps]"
  exit 1
fi
...
info "Running Fortio load: mode=${MODE} duration=${DURATION} concurrency=${CONCURRENCY} qps=${QPS}"
info "Target: ${TARGET}"
